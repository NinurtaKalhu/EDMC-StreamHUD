...
def parse_latest_journals_for_unsold():
    ...
    try:
        journals = [os.path.join(saved_games, f) for f in os.listdir(saved_games) if f.startswith("Journal.") and f.endswith(".log")]
        if not journals:
            return
...
def format_exploration_risk() -> str:
    parts = []
    if saved_state["unsold_elw"] > 0: parts.append(f"ELW: {saved_state['unsold_elw']}")
    if saved_state["unsold_ww"] > 0: parts.append(f"WW: {saved_state['unsold_ww']}")
    if saved_state["unsold_aw"] > 0: parts.append(f"AW: {saved_state['unsold_aw']}")
...
def generate_html():
    ...
    bg_css = f"background: rgba({r}, {g}, {b}, {opacity:.2f});" if cfg.get("bg_enabled", True) else "background: transparent;"
    shadow_css = "text-shadow: 0 0 5px rgba(0,0,0,0.9), 0 0 2px rgba(0,0,0,0.9);" if cfg.get("text_shadow", True) else ""
...
def plugin_start3(plugin_dir: str) -> str:
    ...
    generate_html()
    update_hud_file()
    print(f"[{plugin_name}] Compact Streamer Module active.")
    return plugin_name
...
def plugin_prefs(parent: nb.Notebook, cmdr: str, is_beta: bool) -> Optional[tk.Frame]:
    try:
        ...
        td_row = 0
        def next_td_row(): nonlocal td_row; td_row += 1; return td_row
        ...
        ts_row = 0
        def next_ts_row(): nonlocal ts_row; ts_row += 1; return ts_row
        ...
        nb.OptionMenu(tab_style, layout_var, layout_var.get(), "vertical", "horizontal").grid(row=r, column=1, sticky=tk.W)
        ...
        def add_color_row(parent_tab, label_text, key):
            ...
            nb.Button(parent_tab, text="Choose", command=lambda v=color_var: v.set(colorchooser.askcolor(color=v.get() or "#FFFFFF", parent=main_frame)[1] or v.get())).grid(row=r, column=2, sticky=tk.W, padx=(4, 10))
        ...
        nb.OptionMenu(tab_style, font_var, font_var.get(), *(FONT_CHOICES if font_var.get() in FONT_CHOICES else [font_var.get()] + FONT_CHOICES)).grid(row=r, column=1, sticky=tk.W)
        ...
        preview_canvas = tk.Canvas(preview_container, width=480, height=160, bg="#3a3a3a", highlightthickness=1, highlightbackground="#888888")
        ...
        def redraw_preview(*_args):
            try:
                ...
                try: font_size_p = int(gv("font_size", 22))
                except (TypeError, ValueError): font_size_p = 22
                try: label_size_p = int(gv("label_font_size", 16))
                except (TypeError, ValueError): label_size_p = 16
                ...
                try: bg_opacity_p = max(0, min(100, int(gv("bg_opacity", 50))))
                except (TypeError, ValueError): bg_opacity_p = 50
                ...
                preview_canvas.create_text(8, canvas_h - 8, anchor="sw", text="(sample background)", fill="#888888", font=("Segoe UI", 8))
                ...
                if not visible_rows:
                    preview_canvas.create_text(canvas_w / 2, canvas_h / 2, text="(no fields selected)", fill="#aaaaaa", font=("Segoe UI", 10))
                    return
                ...
                if bg_enabled_p:
                    ...
                    blended = tuple(round(c * alpha + b * (1 - alpha)) for c, b in zip((br, bgc, bb), backdrop))
                    preview_canvas.create_rectangle(box_x, box_y, box_x + box_w, box_y + box_h, fill="#%02x%02x%02x" % blended, outline="")
                ...
                for dom_id, label_text, sample, row_w, row_h in row_sizes:
                    ...
                    preview_canvas.create_text(lx, ly, text=label_text, anchor="w", fill=label_color_p, font=label_font)
                    lw, _lh = text_size(label_text, label_font)
                    vx = lx + lw + 10

                    if shadow_p:
                        preview_canvas.create_text(vx + 1, ly + 1, text=sample, anchor="w", fill="#000000", font=value_font)
                    preview_canvas.create_text(vx, ly, text=sample, anchor="w", fill=font_color_p, font=value_font)

                    if layout_p == "horizontal": cursor_x += row_w + gap
                    else: cursor_y += row_h + gap
    ...
    try:
        ...
        for key, var in vars_cfg.items():
            ...
            if key in NUMERIC_KEYS:
                try: val = int(val)
                except ValueError: val = DEFAULT_SETTINGS[key]
            new_cfg[key] = val
...
def journal_entry(cmdr: str, is_beta: bool, system: str, station: str, entry: Dict[str, Any], state: Dict[str, Any]):
    try:
        if cmdr: saved_state["cmdr"] = cmdr
        if system: saved_state["system"] = system
        ...
        if event_type == 'docked':
            saved_state['station'] = station
            saved_state['last_docked_ts'] = entry.get('timestamp', datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ"))
            save_totals()
